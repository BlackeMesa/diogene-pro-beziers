      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-4 gap-6 max-w-6xl mx-auto">
          {certifications.map((cert, index) => {
            return (
              <div key={index} className="bg-card rounded-xl p-6 text-center hover:shadow-medium transition-shadow duration-300">
                <div className="flex items-center justify-center h-24 mb-4">
                  <img src={cert.image} alt={cert.title} className="max-h-20 max-w-full object-contain" />
                </div>
                <h3 className="font-bold text-card-foreground mb-2">{cert.title}</h3>
