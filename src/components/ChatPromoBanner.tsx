...
const ChatPromoBanner = ({ variant = "default" }: ChatPromoBannerProps) => {
  ...
  if (variant === "highlight") {
    return (
      <div className="relative overflow-hidden bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 rounded-2xl p-1 shadow-xl animate-pulse-slow">
        <button
          onClick={handleClick}
          className="w-full bg-white dark:bg-gray-900 rounded-xl p-6 flex flex-col sm:flex-row items-center gap-4 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
        >
          ...
          <div className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold px-6 py-3 rounded-lg transition-colors">
            Démarrer le chat
          </div>
        </button>
      </div>
  ...
  return (
    <div className="bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 border-2 border-emerald-200 dark:border-emerald-800 rounded-2xl p-6 shadow-md">
      <button
        onClick={handleClick}
        className="w-full flex flex-col sm:flex-row items-center gap-4 text-left group"
      >
        ...
        <div className="text-center sm:text-left flex-1">
          <p className="text-lg font-bold text-gray-900 dark:text-white mb-1">
            Besoin de parler ? Nous sommes là !
          </p>
          <p className="text-gray-600 dark:text-gray-300 text-sm">
            Posez vos questions en direct, sans engagement. Réponse rapide garantie.
          </p>
        </div>
        <div className="bg-emerald-500 hover:bg-emerald-600 text-white font-semibold px-5 py-2.5 rounded-lg transition-colors whitespace-nowrap">
          Ouvrir le chat
        </div>
      </button>
    </div>
