      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {badges.map((badge, index) => (
            <div key={index} className="bg-card p-6 rounded-xl shadow-soft hover:shadow-medium transition-shadow duration-300">
              <div className="flex flex-col items-center text-center">
                {badge.image ? (
                  <img src={badge.image} alt={badge.title} className="w-16 h-16 mb-4 object-contain" />
                ) : (
                ...
                <h3 className="text-lg font-bold text-card-foreground mb-2">{badge.title}</h3>
